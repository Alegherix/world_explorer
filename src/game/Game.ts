...
abstract class Game implements ISkybox {
  ...
  abstract runGameLoop(timeDelta: number, elapsedTime: number);
  ...
  createPlayer() {
    const startPosition = {
      x: (0.5 - Math.random()) * this.width,
      y: Math.random() * (this.width / 2) + 10,
      z: (0.5 - Math.random()) * this.width,
    };
