...
class Platform {
  ...
  private static addPointsToPlane(dimension: IDimension, position: IPosition, scoreKeeper: ScoreKeeper) {
    ...
    if (height > width) {
      appendage = height / 2;
      for (let index = offset; index < height - offset; index += spaceBetweenPoints) {
        scoreKeeper.createCoin(x, y + offset, z - appendage + index);
      }
    } else {
      for (let index = offset; index < width - offset; index += spaceBetweenPoints) {
