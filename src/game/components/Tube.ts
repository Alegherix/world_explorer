...
import type { IDimension, ITorusDimension, IPosition } from '../../shared/interfaces';
...
class TubeFactory {
  ...
  static createCustomTube(
    dimensions: ITorusDimension,
    phyicsMaterial: Material,
    position?: IPosition,
    configObj?: MeshStandardMaterialParameters
  ): IGamePiece {
    ...
    const shape = Trimesh.createTorus(radius, tube, radialSegments, tubularSegments, arc);
    const body = new Body({
      mass: 0,
      position: new Vec3(5, 160, 0),
      shape,
